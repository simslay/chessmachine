...
public class ArtificialWhite extends Player {
	public static Fevals feval = Fevals.wFeval;
	public static int mode = HORIZON1;
	public static int PMAX = 2;

	public ArtificialWhite() {
		type=ARTIFICIAL;
		color=WHITE;
	}
	...
	public static double fevalCheckmate(Game game, State state) {
		...
		if (GameUtil.blackIsCheckmate(game, new JLabel()))
			res = 100;
	...
	public static double fevalStalemate(Game game, State state) {
		...
		if (GameUtil.blackIsStalemate(game, new JLabel()))
			res = -10;
	...
	public static double fevalPromotion(State state) {
		...
		for (int i=0; i<8; i++) {
			for (int j=0; j<8; j++) {
				piece = board[i][j];
				if (piece != null && piece.getColor() == Game.WHITE && piece.isPromoted()) {
	...
	/* setters */
	public void setFeval(Fevals feval) {ArtificialWhite.feval = feval;}
